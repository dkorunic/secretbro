...
- `cargo clippy --release` — there is no CI lint step, but a `[lints]` table in `Cargo.toml` (clippy `all` + `redundant_clone` = deny, `nonstandard_style` = deny) enforces the baseline at build time; clippy is still the expected sanity check before commits. ⚠️ Lint-check the Linux targets too (`cargo clippy --target {x86_64,aarch64}-unknown-linux-gnu --target x86_64-unknown-linux-musl`): the `#[cfg(target_os = "linux")]` `hook!` arm (with its `dlsym`/`transmute`) is invisible to clippy on a macOS host, so Linux-only lints won't surface unless you cross-check.
- `rust-toolchain.toml` pins channel `stable`. Edition is 2021, MSRV is 1.81.0 (the `#[expect(...)]` lint attribute requires ≥ 1.81).
