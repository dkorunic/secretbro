...
- `cargo fmt` — `rustfmt.toml` enforces `max_width = 79` and `use_small_heuristics = "max"`. `reorder_imports`/`group_imports = "StdExternalCrate"` are also set, but `group_imports` is nightly-only — stable `cargo fmt` warns and ignores it, so run `cargo +nightly fmt` to actually regroup imports. Hook macro invocations have lines that exceed 79 cols and rustfmt does not reformat inside macros — leave them alone.
- `cargo clippy --release` — there is no CI lint step, but a `[lints]` table in `Cargo.toml` (clippy `all` + `redundant_clone` = deny, rust `nonstandard_style` = deny, `future-incompatible` + `rust-2018-idioms` = warn) enforces the baseline at build time; clippy is still the expected sanity check before commits. ⚠️ Lint-check the Linux targets too (`cargo clippy --target {x86_64,aarch64}-unknown-linux-gnu --target x86_64-unknown-linux-musl`): the `#[cfg(target_os = "linux")]` `hook!` arm (with its `dlsym`/`transmute`) is invisible to clippy on a macOS host, so Linux-only lints won't surface unless you cross-check.
- The toolchain channel is not pinned (no `rust-toolchain.toml`); `Cargo.toml` declares edition 2021 and `rust-version = "1.81.0"` (the `#[expect(...)]` lint attribute requires ≥ 1.81).
...
1. **Hooked libc functions** — declared via `hook! { unsafe fn name(...) -> ret => my_name { ... } }` (the macro is defined in `src/hook.rs` and brought into scope by `#[macro_use] mod hook;` at the top of `src/lib.rs`). Hooks fall into two intent buckets crossed with four cfg-gating tiers:
   ...
   *Intent:*
   - **Read** (returns sentinel on hit): `creat`, `open*`, `fopen*` — i.e. the calls that open secret *contents*. Only these reads are hooked.
   ...
   **Metadata-only calls are deliberately NOT hooked** (`stat`/`lstat`/`fstatat`/`statx`/`__xstat*`, `access`/`faccessat`, `readlink`/`readlinkat`, `opendir`, `name_to_handle_at`). Every hook canonicalizes via `realpath`, which is blocking and not async-signal-safe; a host that `stat`s/`access`es on a hot path (e.g. HAProxy's event loop holding a per-SERVER lock) would stall and trip its watchdog ([haproxytech/kubernetes-ingress#818](https://github.com/haproxytech/kubernetes-ingress/issues/818)). Blocking metadata buys little security — a caller still cannot read secret bytes or move/shadow a secret — so the surface is limited to content reads + mutations. Do not re-add metadata hooks without re-opening that trade-off.

   *Gating tiers:*
   - **Cross-platform** (Linux + macOS): the basic POSIX read/modify set (`open`, `creat`, `fopen`, `freopen`, `mkdir`, `rmdir`, `unlink`, `truncate`, `chmod`, `chown`, `lchown`, `utime`, `utimes`, `mknod`, `rename`, `link`, `symlink`) — 18 hooks, matching the `__interpose` entry count asserted in `tests/macos_interpose.rs`.
   - **`#[cfg(target_os = "linux")]`**: the `*at` modify family (`mkdirat`, `unlinkat`, `fchmodat`, `fchownat`, `mknodat`, `utimensat`, `renameat`, `linkat`, `symlinkat`). Linux-only because on x86_64 macOS the relevant real symbols are mangled (`$INODE64` etc.) and a plain hook would not intercept them.
   - **`#[cfg(all(target_os = "linux", target_env = "gnu"))]`**: glibc-only — `renameat2`, and the LFS `*64` content-open variants (`creat64`, `open64`, `openat64`, `fopen64`, `freopen64`) plus `truncate64`. The `dlsym`/`ENOSYS` fallback in `hook.rs` makes a missing symbol a runtime no-op rather than a link error. The `*64` symbols are a glibc convention that does not exist on musl or macOS.

   Each hook returns the libc error sentinel (`-1` for `c_int`, `null_mut()` for `*mut FILE`) when `is_secret_path` (or `either_secret` for two-path ops) rejects the path; otherwise it forwards to `real!(name)(args)`.
...
5. **`hook!` macro internals** — the Linux expansion stamps each hook with a `#[no_mangle] pub unsafe extern "C" fn` plus a per-symbol `OnceLock<fn ptr>` resolved lazily via `dlsym(RTLD_NEXT, "name\0")`. If `dlsym` returns null (symbol absent in the loaded libc — e.g. `renameat2` on glibc < 2.28), the hook installs `__enosys_fallback` which sets `errno = ENOSYS` and returns the type's `HookSentinel` (`-1` for ints/ssize, `null_mut()` for pointers), with a one-line warning emitted to stderr exactly once on first miss. The resolved pointer (the real symbol *or* the fallback) is cached in the `OnceLock`, so a missing symbol is resolved once, not re-`dlsym`'d per call. The macOS expansion is structurally different: a `#[link_section = "__DATA,__interpose"]` static pairs hook and target pointers, processed by dyld at load time — no `dlsym`, no fallback.

6. **Variadic ABI shortcut** — `open`/`openat` and the `*64` variants are variadic in C (`mode` is consumed only when `O_CREAT` is in `flags`), but the `hook!` macro can't model varargs. Each is declared with a fixed `mode: mode_t` parameter; on AMD64 SysV / AArch64 AAPCS, the libc implementations ignore the unused register when `O_CREAT` is absent. Strict ABI mismatch, benign on these targets — do not extend this assumption to other architectures without testing.
...
For glibc-only LFS `*64` variants and glibc-only modern syscalls (`renameat2`), gate on `#[cfg(all(target_os = "linux", target_env = "gnu"))]` (not `target_arch = "x86_64"` — that gate was wrong on both axes, missing aarch64-gnu and emitting dead symbols on x86_64-musl). For Linux-only POSIX `*at` variants, gate on `#[cfg(target_os = "linux")]`. If the symbol may not exist in the loaded libc (e.g. cross-glibc-version compatibility), no extra work is needed — the macro's `dlsym`/`ENOSYS` fallback handles it. The existing hooks are the reference.

Before adding a metadata-only hook (`stat`/`access`/`readlink`/`opendir`/`statx`/…), re-read the intent section above: those were intentionally removed to keep hot-path callers off `realpath` (issue #818). Adding one back is a security-vs-availability decision, not a routine extension.
